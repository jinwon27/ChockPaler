package com.brave.chockpaler.aspect;
...
public class AuthAspect {
	 ...
	 *  [ 이 aspect 가 적용되는 메소드 ]
	 *  
	 *  1. return type 이  ModelAndView
	 *  2. 메소드의 이름이 auth 로 시작하는 메소드
	 *  3. 이 aspect 가 정상 동작하기 위해서는 메소드의 인자중에 HttpServletRequest 객체가 있어야한다.
	 *  
	 *  - 이 aspect 는 Controller 의 특정 메소드에 적용이 되게해서 특정 요청이 로그인을 하지 않은 상태로
	 *  된다면 로그인 폼 요청으로 리다이렉트 시키는 aspect 이다.
	 */
