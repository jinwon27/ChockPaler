package com.brave.chockpaler.util;

public class reviewPageUtil {
	// 페이지당 개시물 수
	...
	// 생성자
	// pageUtil(리스트 갯수, 현재 페이지 번호)
	public reviewPageUtil(int count, int curPage) {
	...
	public void setPageRange() {
		...
		// WHERE rn BETWEEN #{start} AND #{end}
		// 시작번호 = (현재페이지-1)*페이지당 개시물 수+1
		pageBegin = (curPage - 1) * PAGE_SCALE + 1;
		// 끝번호 = 시작번호+페이지당 개시물 수-1
	...
	public void setTotBlock() {
		// 전체 페이지 갯수 / 10
		// 91 / 10 => 9.1 => 10개
		totBlock = (int) Math.ceil(totPage / PAGE_SCALE);
	}
	...
	public String toString() {
		return "pageUtil [curPage=" + curPage + ", prevPage=" + prevPage + ", nextPage=" + nextPage + ", totPage="
